<template>
                            ...
                            <div class="text-center">
                                ...
                                <div class="p-2 mt-4">
                                    <form @submit.prevent="preview" enctype="multipart/form-data">
                                        <h5>Sync Qualifiers</h5>
                                        <p class="text-muted">Please click to sync to download qualifiers from central server.</p>
                                        
                                
                                        <div class="mt-4">
                                            <button type="submit" class="btn btn-success w-lg me-1">Sync</button>
                                            <Link href="/qualifiers" class="btn btn-light w-lg">Back</Link>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        ...
                        <table class="table table-centered align-middle table-nowrap mb-0">
                            <tbody>
                                <tr v-for="user in users" v-bind:key="user.spas_id" :id="user.spas_id">
                                    ...
                                    <td>
                                        <h5 class="font-size-13 mb-0 text-dark">{{user.lastname}}, {{user.firstname}} {{user.middlename[0]}}. <span class="text-muted font-size-10">({{ user.spas_id }})</span></h5>
                                        <p class="font-size-11 text-muted mb-0">{{user.municipality.name}}, {{user.province.name}},  {{user.region.region }}</p>
                                    </td>
                                    ...
                                    <td class="text-center fw-bold text-primary">{{user.year}}</td>
    ...
    export default {
        ...
        methods : {
            ...
            preview(){
                this.isLoading = true;
                axios.get(this.currentUrl + '/qualifiers/fetch')
                .then(response => {
